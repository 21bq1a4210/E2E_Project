...
def view_complaints(request):
    ...
    complaints: Complaint = Complaint.objects.all().order_by('-complaint_time', '-complaint_date')[:30]
...
def escalate_complaint(request):
    if request.method == 'PUT':
        complaint_id = json.loads(request.body)['complaint_id']
